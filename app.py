...
def create_pulseira_image(patient_data, logo_image=None, fonts=None):
    ...
    # Data/hora e médico em azul escuro, fonte maior e negrito, posicionados no canto inferior direito da área imprimível
    ts = datetime.now().strftime('%d/%m/%Y %H:%M')
    medico = str(patient_data.get('Médico responsável', '')).strip()
    try:
        font_bold_big = ImageFont.truetype(FONT_BOLD_LOCAL.path if hasattr(FONT_BOLD_LOCAL, 'path') else FONT_BOLD_LOCAL, size=int(FONT_BOLD_LOCAL.size*1.5) if hasattr(FONT_BOLD_LOCAL, 'size') else 28)
    except Exception:
    ...
    ts_x = printable_right - ts_w - cm_to_px(0.2)
    ts_y = P_HEIGHT - cm_to_px(0.15) - ts_h
    ...
    if medico:
        ...
        med_x = printable_right - med_w - cm_to_px(0.2)
        med_y = ts_y - med_h - cm_to_px(0.1)
...
class PulseiraApp:
    def __init__(self, root):
        ...
        # Exportação em A4 (paisagem) com escala física exata
        self.btn_export_pdf_a4 = tb.Button(ctrl_frame, text='Exportar PDF (A4 real)', command=self.export_pdf_a4)
        ...
        # botão para imprimir
        self.btn_print = tb.Button(ctrl_frame, text='Imprimir', command=self.print_pulseira)
    ...
    def print_pulseira(self):
        # Gera a imagem da pulseira usando a função global
        img = create_pulseira_image(self.patient_data, self.logo_image, self.fonts)
        # Salva temporariamente
        import tempfile, os, platform
        with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as tmp:
            img.save(tmp.name, 'PNG')
            tmp_path = tmp.name
        # Comando de impressão para Windows
        if platform.system() == 'Windows':
            os.startfile(tmp_path, 'print')
        else:
            self.status['text'] = 'Impressão automática só disponível no Windows.'
        self.status['text'] = 'Enviado para a impressora.'
